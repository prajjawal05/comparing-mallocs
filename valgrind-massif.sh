...
do
    for bm in "${bms[@]}"
    do
         echo "Running valgrind massif for malloc: $malloc and bm: $bm"
         log_file="results/$bm-$malloc-valgrind-massif.txt"
         touch "$log_file"
         sudo ../../bench.sh --procs=16 "$malloc" "$bm" # requires change in run_test, add valgrind --heap=yes --stacks=no --detailed-freq=100 --tool=massif
         mv massif.out.* "$log_file"
